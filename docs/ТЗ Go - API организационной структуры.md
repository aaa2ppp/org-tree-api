...
* parent\_id: int | null (FK на Department, позволяет строить дерево)
...
* department\_id: int (FK на Department)

* full\_name: str (не пустой)

* position: str (не пустой)

* hired\_at: date | null (опционально)
...
* Department 1—N Department (самоссылка через parent\_id)
...
* Body:
  ...
  * parent\_id: int | null (опционально)
...
* Query:
  ...
  * include\_employees: bool (по умолчанию true)

* Response:

  * department (объект подразделения)

  * employees: \[\] (если include\_employees=true, сортировка по created\_at или full\_name)
...
* Query:
  ...
  * mode: str (cascade | reassign)
    ...
    * reassign — удалить подразделение, а сотрудников перевести в reassign\_to\_department\_id

  * reassign\_to\_department\_id: int (обязателен, если mode=reassign)
